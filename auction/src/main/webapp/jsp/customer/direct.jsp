...
<%@ taglib uri="timeTag" prefix="timetag" %>
<%@ page session="true" %>
...
<fmt:message bundle="${lang}" key="table.header.direct" var="headerDirect"/>
<fmt:message bundle="${lang}" key="user.balance" var="userBalance"/>

<fmt:message bundle="${lang}" key="today.message" var="today"/>


<fmt:setBundle basename="Pages" var="config"/>
<fmt:message bundle="${config}" key="path.page.direct" var="contextPath"/>
...
<div class="row">
        ...
        <ul class="col-12 col-m-12">
            <li class="col-2 col-m-12">
                <form id="reverseAuction" method="POST"
                      action="${pageContext.request.contextPath}/controller?command=reverse">
                    <button type="submit" form="reverseAuction"><c:out value="${reverse}"/></button>
                </form>
            </li>
            <li class="col-2 col-m-12">
    ...
    <div class="col-9 col-m-9">
        <h1><c:out value="${headerDirect}"/></h1>
        <form id="bidForm" method="POST" action="${pageContext.request.contextPath}/controller?command=direct_bid">
            <table class="bidTable">
                ...
                <c:forEach var="elem" items="${lst}" varStatus="status">
                    <tr name="${elem.lotName}" onclick="radiobuttonClick(this)">
                        ...
                        <td hidden><input id="${elem.lotName}" type="radio" name="bid" value="${elem.id}"/>СТАВКА</td>
                    </tr>
                </c:forEach>
                </tbody>
                <tfoot>
    ...
    <div class="col-3 col-m-12">
        <div class="aside">
            ...
            <c:out value="${today}"/> <timetag:today format="dd. MM. yyyy"/>
        </div>
    </div>
</div>
